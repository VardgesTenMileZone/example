...
} from "../../../store/actions/myTripAction";
import './modal.scss';
import _ from 'lodash';
import Stripe from "../Stripe/Stripe";
import ChangeTicketDatesTable from '../Stripe/ChangeTicketDatesTable';
...
class ChangeTicketDates extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      modifyTicketOptionsResult: null
    }
  }

  modifyTicketTypeOptionsChange = e => {
    this.setState({
      modifyTicketOptions: {
        ...this.state.modifyTicketOptions,
        charge: e.target.value
      }
    })
  }

  modifyTicketAmountOptionsChange = e => {
  ...
  componentDidUpdate() {
    const { searchParams, search, loading, loaded, item, depFilters, changeAvailability, availabilityData, changeTicketDatesIsOpen, resetSearchRequest, setResetChangeDates, selectedMountain } = this.props;
    ...
    if (searchParams.from && searchParams.to && searchParams.startDate && !this.state.showLocationInputs) {
      ...
      const toCity = _.find(selectedMountain.goingTo, (i) => i.value === item.to.city);
      const fromCity = _.find(selectedMountain.goingTo, (i) => i.value === item.from.city);
      search(
        {
          from: fromCity.key,
          to: toCity.key,
  ...
  isMatch = (item) => {
    ...
    // get filter conditions from redux
    // const filters = type === 'departure' ? depFilters : retFilters
  ...
  onPuy = () => {
    this.setState({ stripeModalOpen: !this.state.stripeModalOpen })
  }
  ...
  changeTicketWithPayment = () => {
    const { modifyTicketOptions } = this.state;
    let data = {}

    if(modifyTicketOptions.charge !== 'default'){
      data.charge = modifyTicketOptions.charge === 'With Charge' ? true : false;

      if(modifyTicketOptions.charge === 'With Charge'){
        data.amount = modifyTicketOptions.amount;
      }

      if(modifyTicketOptions.charge === 'Without Charge'){
        return this.changeTicketForFree(data)
      }
    }

    this.setState({ stripeModalOpen: true, modifyTicketOptionsResult: Object.keys(data).length > 0 ? data: null });
  }

  changeTicketForFree = (dataChange = null) => {
    const data = {
      date: this.state.selectedTicket.date,
      uuid: this.props.item.uuid,
      // token: res.id,
  ...
  onStripeCallback = async (res, amount, stripeError) => {
    ...
    if (res && res.id) {
      const { modifyTicketOptionsResult } = this.state;
      ...
      if(modifyTicketOptionsResult){
        data.charge = modifyTicketOptionsResult.charge;

        if(modifyTicketOptionsResult.amount){
          data.amount = Number(modifyTicketOptionsResult.amount) * 100;
        }
      }
  ...
  render() {
    ...
    console.log(this.state.modifyTicketOptionsResult);
            ...
            <ModalBody>
              ...
              {this.state.changeTicketWithPayment &&
                <div className="change-ticket-with-payment">
                  ...
                  <div className="modify-ticket-options row">
                    <div className="col-sm-5">
                      <Input type="select" onChange={this.modifyTicketTypeOptionsChange} name="select">
                    ...
                    <div className="col-sm-7">
                      {
                        this.state.modifyTicketOptions.charge === 'With Charge' && (
                          <Input
                            onChange={this.modifyTicketAmountOptionsChange}
                            placeholder="value"
                            value={this.state.modifyTicketOptions.amount}
                            type="number"
          ...
          {this.state.stripeModalOpen && (
            <div className="price-stripe-modal-container">
              <div className="price-stripe-modal">
                <Stripe
                  onPuy={this.onPuy}
                  amount={(modifyTicketOptionsResult && modifyTicketOptionsResult.charge && modifyTicketOptionsResult.amount) ? modifyTicketOptionsResult.amount : this.props.changeAvailability.paymentAmount.value}
